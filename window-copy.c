...
#ifdef ENABLE_CAPTURE_MODE
static void window_copy_capture_start(struct window_mode_entry *,
                                      struct client *);
...
static void window_copy_capture_key(struct window_mode_entry *, struct client *,
                                    key_code);
static void window_copy_capture_draw_label(struct screen_write_ctx *,
                                           struct capture_hint *, int,
                                           const struct grid_cell *,
                                           const struct grid_cell *, u_int);
static void window_copy_capture_draw(struct window_mode_entry *,
...
static u_int window_copy_cursor_limit(struct window_mode_entry *, u_int, int);
...
static void window_copy_cursor_previous_word(struct window_mode_entry *,
                                             const char *, int);
static void window_copy_cursor_prompt(struct window_mode_entry *, int, int);
 ...
 */
static void window_copy_sync_snapshot(struct window_copy_mode_data *data,
                                      struct grid *src) {
 ...
 */
static int window_copy_sync_backing(struct window_mode_entry *wme) {
  struct window_copy_mode_data *data = wme->data;
  ...
  if (added == 0 && collected == 0) {
    ...
    if (new_hsize + sy != old_hsize + sy - collected) {
      dg->linedata =
          xreallocarray(dg->linedata, new_hsize + sy, sizeof *dg->linedata);
      memset(&dg->linedata[old_hsize + sy - collected], 0,
...
static struct screen *window_copy_init(struct window_mode_entry *wme,
  ...
  data->screen.cx =
      window_copy_cursor_offset(wme, data->cx, screen_size_x(&data->screen));
...
static void window_copy_free(struct window_mode_entry *wme) {
  ...
  free(data->capture_hint_keys);
  if (data->capture_key_table != NULL) {
    key_bindings_remove_table(data->capture_key_table);
    free(data->capture_key_table);
  }
#endif
...
void window_copy_vadd(struct window_pane *wp, int parse, const char *fmt,
  ...
  if (parse) {
    ...
    input_parse_screen(data->ictx, backing, window_copy_init_ctx_cb, data, text,
                       strlen(text));
    free(text);
  } else {
...
void window_copy_scroll(struct window_pane *wp, int sl_mpos, u_int my,
...
static void window_copy_scroll1(struct window_mode_entry *wme,
                                struct window_pane *wp, int sl_mpos, u_int my,
                                u_int tty_oy, int scroll_exit) {
  struct window_copy_mode_data *data = wme->data;
...
char *window_copy_get_hyperlink(struct window_pane *wp, u_int x, u_int y) {
  struct window_mode_entry *wme = TAILQ_FIRST(&wp->modes);
  struct window_copy_mode_data *data = wme->data;
...
static void *window_copy_cursor_hyperlink_cb(struct format_tree *ft) {
  ...
  return (
      format_grid_hyperlink(gd, data->cx, gd->hsize + data->cy, &data->screen));
}
...
static struct screen *window_copy_get_screen(struct window_mode_entry *wme) {
  struct window_copy_mode_data *data = wme->data;
...
static const char *window_copy_key_table(struct window_mode_entry *wme) {
  ...
  if (data->capture_active && data->capture_key_table != NULL)
    return (data->capture_key_table);
#endif
...
window_copy_cmd_search_backward_incremental(struct window_copy_cmd_state *cs) {
  ...
  if (data->searchx == -1 || data->searchy == -1) {
    ...
    data->cx = window_copy_cursor_limit(
        wme, screen_hsize(data->backing) + data->cy - data->oy, 0);
 ...
 */
static void window_copy_do_refresh(struct window_mode_entry *wme, int follow) {
  struct window_pane *wp = wme->swp;
  struct window_copy_mode_data *data = wme->data;
  u_int oy_from_top;
  ...
  if (follow) {
    data->cy = screen_size_y(&data->screen) - 1;
    data->cx = window_copy_cursor_limit(
        wme, screen_hsize(data->backing) + data->cy, 0);
    data->oy = 0;
...
static void window_copy_refresh_arm(struct window_mode_entry *wme) {
  struct window_copy_mode_data *data = wme->data;
  struct timeval tv = {.tv_sec = WINDOW_COPY_REFRESH_INTERVAL / 1000000,
...
static void window_copy_refresh_timer(__unused int fd, __unused short events,
                                      void *arg) {
  struct window_mode_entry *wme = arg;
  struct window_pane *wp = wme->wp;
  struct window_copy_mode_data *data = wme->data;
  ...
  if ((wp->flags & PANE_UNSEENCHANGES) && data->screen.sel == NULL &&
      data->cursordrag == CURSORDRAG_NONE) {
    follow = (data->oy == 0 && data->cy == screen_size_y(&data->screen) - 1);
...
static void window_copy_refresh_start(struct window_mode_entry *wme) {
  struct window_copy_mode_data *data = wme->data;
...
static void window_copy_refresh_stop(struct window_mode_entry *wme) {
  struct window_copy_mode_data *data = wme->data;
...
#ifdef ENABLE_CAPTURE_MODE
static enum window_copy_cmd_action
window_copy_cmd_capture(struct window_copy_cmd_state *cs) {
  window_copy_capture_start(cs->wme, cs->c);
  return (WINDOW_COPY_CMD_REDRAW);
}

static enum window_copy_cmd_action
window_copy_cmd_capture_key(struct window_copy_cmd_state *cs) {
  struct window_copy_mode_data *data = cs->wme->data;
  const char *arg0 = args_string(cs->wargs, 0);
...
static enum window_copy_cmd_action
window_copy_cmd_capture_cancel(struct window_copy_cmd_state *cs) {
  window_copy_capture_stop(cs->wme, cs->c);
  return (WINDOW_COPY_CMD_REDRAW);
}
...
#define WINDOW_COPY_CMD_FLAG_READONLY 0x1
  ...
  enum window_copy_cmd_action (*f)(struct window_copy_cmd_state *);
    ...
    {.command = "capture",
     ...
     .f = window_copy_cmd_capture},
    {.command = "capture-key",
     .args = {"", 1, 1, NULL},
     .flags = WINDOW_COPY_CMD_FLAG_READONLY,
     .clear = WINDOW_COPY_CMD_CLEAR_NEVER,
     .f = window_copy_cmd_capture_key},
    {.command = "capture-cancel",
     .args = {"", 0, 0, NULL},
     .flags = WINDOW_COPY_CMD_FLAG_READONLY,
     .clear = WINDOW_COPY_CMD_CLEAR_NEVER,
     .f = window_copy_cmd_capture_cancel},
#endif
  ...
  for (i = 0; i < nitems(window_copy_cmd_table); i++) {
    if (strcmp(window_copy_cmd_table[i].command, command) == 0) {
      ...
      if (c != NULL && c->flags & CLIENT_READONLY &&
          (~flags & WINDOW_COPY_CMD_FLAG_READONLY)) {
        status_message_set(c, -1, 1, 0, 0, "client is read-only");
        return;
      }

      cs.wargs = args_parse(&window_copy_cmd_table[i].args, args_values(args),
                            count, &error);
...
static u_int window_copy_clip_width(u_int width, u_int b, u_int sx, u_int sy) {
  return ((b + width > sx * sy) ? (sx * sy) - b : width);
}

static u_int window_copy_search_mark_match(struct window_copy_mode_data *data,
                                           u_int px, u_int py, u_int width,
                                           int regex) {
  struct grid *gd = data->backing->grid;
  struct grid_cell gc;
...
again:
  ...
  for (py = start; py < end; py++) {
    ...
    for (;;) {
      if (regex) {
        found = window_copy_search_lr_regex(gd, &px, &width, py, px, sx, &reg);
        ...
        found = window_copy_search_lr(gd, ssp->grid, &px, py, px, sx, cis);
        if (!found)
          break;
      }
      nfound++;
...
static void window_copy_write_one(struct window_mode_entry *wme,
                                  struct screen_write_ctx *ctx, u_int px,
                                  u_int py, u_int fy, u_int nx,
                                  const struct grid_cell *mgc,
                                  const struct grid_cell *cgc,
                                  const struct grid_cell *mkgc) {
  struct window_copy_mode_data *data = wme->data;
...
static int window_copy_line_number_mode(struct window_mode_entry *wme) {
  struct window_pane *wp = wme->wp;
  struct window_copy_mode_data *data = wme->data;
...
static int window_copy_line_number_is_absolute(struct window_mode_entry *wme) {
...
static int window_copy_line_numbers_active(struct window_mode_entry *wme) {
  return (window_copy_line_number_mode(wme) != WINDOW_COPY_LINE_NUMBERS_OFF);
}

static u_int window_copy_line_number_width(struct window_mode_entry *wme) {
  struct window_copy_mode_data *data = wme->data;
  u_int lines, digits;
...
static u_int window_copy_cursor_offset(struct window_mode_entry *wme, u_int cx,
                                       u_int sx) {
...
static u_int window_copy_cursor_unoffset(struct window_mode_entry *wme,
                                         u_int vx, u_int sx) {
  u_int width = window_copy_line_number_width(wme);
  u_int content;
...
void window_copy_set_line_numbers(struct window_pane *wp, int enabled) {
  struct window_mode_entry *wme = TAILQ_FIRST(&wp->modes);
  struct window_copy_mode_data *data;
...
int window_copy_get_current_offset(struct window_pane *wp, u_int *offset,
                                   u_int *size) {
  struct window_mode_entry *wme = TAILQ_FIRST(&wp->modes);
  struct window_copy_mode_data *data = wme->data;
  u_int hsize;
...
static void window_copy_write_line(struct window_mode_entry *wme,
  ...
  if (width != 0) {
    ...
    style_apply(&cur_ln_gc, oo, "copy-mode-current-line-number-style", ft);
    ...
    screen_write_nputs(ctx, width, current ? &cur_ln_gc : &ln_gc, "%*u ",
                       (int)width - 1, line_number);
  }

  window_copy_write_one(wme, ctx, width, py, hsize - data->oy + py, content_sx,
                        &mgc, &cgc, &mkgc);
  ...
  if (py == data->cy && data->cx >= content_sx) {
    screen_write_cursormove(
        ctx, window_copy_cursor_offset(wme, data->cx, screen_size_x(s)), py, 0);
...
static void window_copy_redraw_lines(struct window_mode_entry *wme, u_int py,
  ...
  if (window_copy_line_number_width(wme) != 0) {
    ...
    for (i = py; i < py + ny; i++)
      window_copy_write_line(wme, &ctx, i);
#ifdef ENABLE_CAPTURE_MODE
    window_copy_capture_draw(wme, &ctx);
#endif
    screen_write_cursormove(
        &ctx, window_copy_cursor_offset(wme, data->cx, screen_size_x(s)),
        data->cy, 0);
    screen_write_stop(&ctx);
...
static void window_copy_style_changed(struct window_mode_entry *wme) {
  struct window_copy_mode_data *data = wme->data;
...
static int window_copy_set_selection(struct window_mode_entry *wme,
  ...
  screen_set_selection(s, sx, sy, endsx, endsy, data->rectflag, clipx,
                       data->modekeys, &gc);
...
static void window_copy_copy_buffer(struct window_mode_entry *wme,
  ...
  if (set_clip && options_get_number(global_options, "set-clipboard") != 0) {
    if (window_copy_line_numbers_active(wme) && (wp->flags & PANE_REDRAW)) {
...
#ifdef ENABLE_CAPTURE_MODE
/* Free all capture matches, hints and reset the input buffer. */
static void window_copy_capture_clear(struct window_copy_mode_data *data) {
  capture_match_free(data->capture_matches, data->capture_match_count);
  ...
  capture_pattern_free(&data->capture_pattern);
 ...
 */
static char *window_copy_capture_pattern(struct options *oo) {
  const char *extra;
  char *pattern;

  if (options_get(oo, "@capture-patterns-override") != NULL) {
    return (xstrdup(options_get_string(oo, "@capture-patterns-override")));
  }

  if (options_get(oo, "@capture-patterns") != NULL) {
    extra = options_get_string(oo, "@capture-patterns");
    if (*extra != '\0') {
      xasprintf(&pattern, "%s|%s", CAPTURE_DEFAULT_PATTERNS, extra);
      return (pattern);
    }
  }
 ...
 */
static void window_copy_capture_bind_keys(struct window_copy_mode_data *data) {
  struct cmd_parse_result *pr;
  struct cmd_list *cmdlist;
  const char *keys = data->capture_hint_keys;
  char *cmd;
  u_int i;

  for (i = 0; keys[i] != '\0'; i++) {
    xasprintf(&cmd, "send -X capture-key -- %c", keys[i]);
    ...
    /*
     * A freshly parsed cmdlist has one reference; key_bindings_add
     * takes ownership of it, so no extra reference is needed for a
     * single binding.
     */
    key_bindings_add(data->capture_key_table, keys[i], NULL, 0, pr->cmdlist);
  }

  /*
   * The cancel cmdlist is shared by three bindings, so it needs one
   * reference per binding.
   */
  pr = cmd_parse_from_string("send -X capture-cancel", NULL);
  if (pr->status == CMD_PARSE_SUCCESS) {
    cmdlist = pr->cmdlist;
    key_bindings_add(data->capture_key_table, 'q', NULL, 0, cmdlist);
    cmdlist->references++;
    key_bindings_add(data->capture_key_table, '\033', NULL, 0, cmdlist);
    cmdlist->references++;
    key_bindings_add(data->capture_key_table, 'c' | KEYC_CTRL, NULL, 0,
                     cmdlist);
  } else
    free(pr->error);
}
 ...
 */
static void window_copy_capture_start(struct window_mode_entry *wme,
                                      struct client *c) {
  struct window_pane *wp = wme->wp;
  struct window_copy_mode_data *data = wme->data;
  struct options *oo = wp->window->options;
  struct grid *gd = data->backing->grid;
  struct capture_match *lm;
  ...
  /* Compile the pattern; on failure leave the overlay disarmed. */
  pattern_str = window_copy_capture_pattern(oo);
  if (capture_pattern_compile(&data->capture_pattern, pattern_str) != 0) {
    log_debug("%s: invalid capture pattern", __func__);
    free(pattern_str);
    return;
  }
  free(pattern_str);

  /* Resolve the hint key set once for this session. */
  free(data->capture_hint_keys);
  if (options_get(oo, "@capture-hint-keys") != NULL)
    hint_keys = options_get_string(oo, "@capture-hint-keys");
  else
  ...
  for (row = 0; row < sy; row++) {
    py = hsize - data->oy + row;
    line = grid_string_cells(gd, 0, py, sx, NULL, 0, NULL);
    if (line == NULL)
      continue;
    line_n = 0;
    lm = capture_pattern_match_line(&data->capture_pattern, line, row, &line_n);
    if (line_n > 0) {
      data->capture_matches = xrecallocarray(
          data->capture_matches, data->capture_match_count,
          data->capture_match_count + line_n, sizeof *data->capture_matches);
      memcpy(data->capture_matches + data->capture_match_count, lm,
             line_n * sizeof *lm);
  ...
  capture_pattern_resolve_overlaps(data->capture_matches,
                                   &data->capture_match_count);

  if (data->capture_match_count == 0) {
    window_copy_capture_clear(data);
    if (c != NULL)
      status_message_set(c, -1, 1, 0, 0, "no matches");
    return;
  }

  data->capture_hints =
      capture_hint_generate(data->capture_match_count, data->capture_hint_keys,
                            &data->capture_hint_count);
  if (data->capture_hints == NULL) {
    window_copy_capture_clear(data);
    if (c != NULL)
      status_message_set(c, -1, 1, 0, 0, "too many matches");
    return;
  }

  /* Build the key table and switch the client into it. */
  if (data->capture_key_table == NULL) {
    xasprintf(&data->capture_key_table, "capture-mode-%%%u", wp->id);
  } else
    key_bindings_remove_table(data->capture_key_table);
  window_copy_capture_bind_keys(data);

  data->capture_input_len = 0;
  data->capture_active = 1;
  if (c != NULL)
    server_client_set_key_table(c, data->capture_key_table);
}

/* Disarm the overlay and restore the normal copy-mode key table. */
static void window_copy_capture_stop(struct window_mode_entry *wme,
                                     struct client *c) {
  struct window_copy_mode_data *data = wme->data;
 ...
 */
static void window_copy_capture_key(struct window_mode_entry *wme,
  ...
  prefixes = 0;
  for (i = 0; i < data->capture_hint_count; i++) {
    h = &data->capture_hints[i];
    if (h->len < data->capture_input_len)
      continue;
    if (memcmp(h->label, data->capture_input,
    ...
    if (h->len == data->capture_input_len) {
      ...
      set_paste = 1;
      set_clip = 1;
      window_copy_copy_buffer(wme, NULL, xstrdup(buf), len, set_paste,
                              set_clip);
  ...
  if (prefixes == 0) {
    /* No hint matches the input so far: reset and redraw. */
    data->capture_input_len = 0;
  }
  window_copy_redraw_screen(wme);
}
 ...
 */
static void window_copy_capture_draw_label(struct screen_write_ctx *ctx,
                                           struct capture_hint *h, int typed,
                                           const struct grid_cell *hgc,
                                           const struct grid_cell *tgc,
                                           u_int avail) {
  u_int i;

  for (i = 0; i < (u_int)h->len && i < avail; i++) {
    if ((int)i < typed)
      screen_write_putc(ctx, tgc, (u_char)h->label[i]);
    else
      screen_write_putc(ctx, hgc, (u_char)h->label[i]);
  }
}
 ...
 */
static void window_copy_capture_draw(struct window_mode_entry *wme,
                                     struct screen_write_ctx *ctx) {
  struct window_pane *wp = wme->wp;
  struct window_copy_mode_data *data = wme->data;
  struct screen *s = &data->screen;
  ...
  for (i = 0; i < n; i++) {
    ...
    if (data->capture_input_len > 0 &&
        (h->len < data->capture_input_len ||
         memcmp(h->label, data->capture_input,
                (size_t)data->capture_input_len) != 0))
      continue;
    ...
    window_copy_capture_draw_label(ctx, h, data->capture_input_len, &hgc, &tgc,
                                   sx - m->sx);
  }
}
#endif /* ENABLE_CAPTURE_MODE */
...
static void window_copy_copy_selection(struct window_mode_entry *wme,
                                       const char *prefix, int set_paste,
                                       int set_clip) {
  char *buf;
  size_t len;
...
static u_int window_copy_cursor_limit(struct window_mode_entry *wme, u_int py,
                                      int allow_onemore) {
  struct options *oo = wme->wp->window->options;
  u_int len;

  len = window_copy_find_length(wme, py);
  if (allow_onemore || options_get_number(oo, "mode-keys") != MODEKEY_VI)
    return (len);
  if (len == 0)
    return (0);
  return (len - 1);
...
static void window_copy_cursor_prompt(struct window_mode_entry *wme,
                                      int direction, int start_output) {
  struct window_copy_mode_data *data = wme->data;
  struct screen *s = data->backing;
  struct grid *gd = s->grid;
  u_int end_line;
