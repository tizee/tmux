...
static void
window_copy_capture_clear(struct window_copy_mode_data *data)
{
 ...
 */
static void
window_copy_capture_bind_keys(struct window_copy_mode_data *data)
{
	...
	for (i = 0; keys[i] != '\0'; i++) {
		 ...
		 * single binding.
		 */
		key_bindings_add(data->capture_key_table, keys[i], NULL, 0,
		    pr->cmdlist);
	}
	...
	if (pr->status == CMD_PARSE_SUCCESS) {
		cmdlist = pr->cmdlist;
		key_bindings_add(data->capture_key_table, 'q', NULL, 0,
		    cmdlist);
		cmdlist->references++;
		key_bindings_add(data->capture_key_table, '\033', NULL, 0,
		    cmdlist);
		cmdlist->references++;
		key_bindings_add(data->capture_key_table, 'c' | KEYC_CTRL, NULL,
		    0, cmdlist);
	} else
		free(pr->error);
}
 ...
 */
static void
window_copy_capture_start(struct window_mode_entry *wme, struct client *c)
{
	struct window_pane		*wp = wme->wp;
	struct window_copy_mode_data	*data = wme->data;
	struct options			*oo = wp->window->options;
	...
	for (row = 0; row < sy; row++) {
		...
		if (line_n > 0) {
			data->capture_matches = xrecallocarray(
			    ...
			    sizeof *data->capture_matches);
			memcpy(data->capture_matches +
			    data->capture_match_count, lm, line_n * sizeof *lm);
			data->capture_match_count += line_n;
			free(lm);
		}
		free(line);
...
static void
window_copy_capture_stop(struct window_mode_entry *wme, struct client *c)
{
	struct window_copy_mode_data	*data = wme->data;
...
window_copy_capture_draw_label(struct screen_write_ctx *ctx,
    struct capture_hint *h, int typed, const struct grid_cell *hgc,
    const struct grid_cell *tgc, u_int avail)
{
	u_int	i;
