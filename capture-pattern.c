 ...
 */
int
capture_pattern_compile(struct capture_pattern *cp, const char *pattern)
{
	int	 rc;
	char	 errbuf[256];
...
void
capture_pattern_free(struct capture_pattern *cp)
{
	if (cp->valid)
		regfree(&cp->re);
	cp->valid = 0;
...
capture_pattern_match_line(struct capture_pattern *cp, const char *line,
    u_int line_no, int *count)
{
	...
	offset = 0;
	while (offset < len &&
	    regexec(&cp->re, line + offset, 1, &pmatch,
 ...
 */
static int
capture_match_overlaps(const struct capture_match *a,
 ...
 */
static u_int
capture_match_length(const struct capture_match *m)
{
	return (m->ex - m->sx);
}
 ...
 */
void
capture_pattern_resolve_overlaps(struct capture_match *matches, int *count)
{
	int i, j, n;
...
void
capture_match_free(struct capture_match *matches, int count)
{
	int i;
