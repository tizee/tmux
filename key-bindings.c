...
#define DEFAULT_SESSION_MENU                                                   \
  " #{S/t:#{?#{&&:#{<:#{loop_index},6},#{!:#{session_active}}},'Switch To "    \
  "#[underscore]#{session_name}' '' {switch-client -t=#{session_id}#} ,}}"     \
  " ''"                                                                        \
...
#define DEFAULT_PANE_MENU                                                      \
  ...
  "history-top}"                                                               \
  " '#{?#{m/r:(copy|view)-mode,#{pane_mode}},Go To Bottom,}' '>' {send -X "    \
  "history-bottom}"                                                            \
  " ''"                                                                        \
  " '#{?#{&&:#{buffer_size},#{!:#{pane_in_mode}}},Paste "                      \
  "#[underscore]#{=/9/...:buffer_sample},}' 'p' {paste-buffer}"                \
  " ''"                                                                        \
  " '#{?mouse_word,Search For #[underscore]#{=/9/...:mouse_word},}' 'C-r' "    \
  "{if -F '#{?#{m/r:(copy|view)-mode,#{pane_mode}},0,1}' 'copy-mode -t='; "    \
  "send -Xt= search-backward -- \"#{q:mouse_word}\"}"                          \
  " '#{?mouse_word,Type #[underscore]#{=/9/...:mouse_word},}' 'C-y' "          \
  "{copy-mode -q; send-keys -l -- \"#{q:mouse_word}\"}"                        \
  " '#{?mouse_word,Copy #[underscore]#{=/9/...:mouse_word},}' 'c' {copy-mode " \
  "-q; set-buffer -- \"#{q:mouse_word}\"}"                                     \
  " '#{?mouse_line,Copy Line,}' 'l' {copy-mode -q; set-buffer -- "             \
  "\"#{q:mouse_line}\"}"                                                       \
  " ''"                                                                        \
  " '#{?mouse_hyperlink,Type #[underscore]#{=/9/...:mouse_hyperlink},}' "      \
  "'C-h' {copy-mode -q; send-keys -l -- \"#{q:mouse_hyperlink}\"}"             \
  ...
  "'d' {swap-pane -D}"                                                         \
  " '#{?#{!:#{pane_floating_flag}},#{?pane_marked_set,,-}Swap Marked,}' 's' "  \
  "{swap-pane}"                                                                \
  " ''"                                                                        \
  " 'Kill' 'X' {kill-pane}"                                                    \
...
static int key_table_cmp(struct key_table *table1, struct key_table *table2) {
  return (strcmp(table1->name, table2->name));
}

static int key_bindings_cmp(struct key_binding *bd1, struct key_binding *bd2) {
  if (bd1->key < bd2->key)
    return (-1);
  if (bd1->key > bd2->key)
    return (1);
...
static void key_bindings_free(struct key_binding *bd) {
...
struct key_table *key_bindings_next_table(struct key_table *table) {
...
struct key_binding *key_bindings_get_default(struct key_table *table,
                                             key_code key) {
  struct key_binding bd;
...
struct key_binding *key_bindings_first(struct key_table *table) {
  return (RB_MIN(key_bindings, &table->key_bindings));
}

struct key_binding *key_bindings_next(__unused struct key_table *table,
                                      struct key_binding *bd) {
  return (RB_NEXT(key_bindings, &table->key_bindings, bd));
}

void key_bindings_add(const char *name, key_code key, const char *note,
                      int repeat, struct cmd_list *cmdlist) {
...
void key_bindings_reset(const char *name, key_code key) {
  struct key_table *table;
...
void key_bindings_reset_table(const char *name) {
  struct key_table *table;
...
static enum cmd_retval key_bindings_init_done(__unused struct cmdq_item *item,
                                              __unused void *data) {
  struct key_table *table;
  ...
  RB_FOREACH(table, key_tables, &key_tables) {
    RB_FOREACH(bd, key_bindings, &table->key_bindings) {
      ...
      RB_INSERT(key_bindings, &table->default_key_bindings, new_bd);
    }
  }
...
void key_bindings_init(void) {
  static const char *const defaults[] = {
      ...
      "bind -N 'Rename current session' '$' { command-prompt -I'#S' { "
      "rename-session -- '%%' } }",
      "bind -N 'Split window horizontally' % { split-window -h }",
      "bind -N 'Kill current window' & { confirm-before -p\"kill-window #W? "
      "(y/n)\" kill-window }",
      "bind -N 'Prompt for window index to select' \"'\" { command-prompt -T "
      "window-target -pindex { select-window -t ':%%' } }",
      ...
      "bind -N 'Rename current window' , { command-prompt -I'#W' { "
      "rename-window -- '%%' } }",
      "bind -N 'Delete the most recent paste buffer' - { delete-buffer }",
      "bind -N 'Move the current window' . { command-prompt -T target { "
      "move-window -t '%%' } }",
      "bind -N 'Describe key binding' '/' { command-prompt -kpkey  { list-keys "
      "-1N '%%' } }",
...
#endif
      "bind -N 'Switch to a window' Tab { new-pane -E -x75% -y30% -X0 -Y0; "
      "move-pane -P bottom-centre; switch-mode -wk }",
      "bind -N 'Switch to a session' BTab { new-pane -E -x75% -y30% -X0 -Y0; "
      "move-pane -P bottom-centre; switch-mode -sk }",
      "bind -N 'Choose a window from a list' w { choose-tree -Zw }",
      "bind -N 'Kill the active pane' x { confirm-before -p\"kill-pane #P? "
      "(y/n)\" kill-pane }",
      "bind -N 'Zoom the active pane' z { resize-pane -Z }",
      "bind -N 'Move pane to top-left corner' '{' { resize-pane -x50% -y50%; "
      "move-pane -P top-left }",
      "bind -N 'Move pane to top-right corner' '}' { resize-pane -x50% -y50%; "
      "move-pane -P top-right }",
      "bind -N 'Move pane to bottom-left corner' 'M-{' { resize-pane -x50% "
      "-y50%; move-pane -P bottom-left }",
      "bind -N 'Move pane to bottom-right corner' 'M-}' { resize-pane -x50% "
      "-y50%; move-pane -P bottom-right }",
      ...
      "}",
      "bind -N 'Select the pane below the active pane' -r Down { select-pane "
      "-D }",
      "bind -N 'Select the pane to the left of the active pane' -r Left { "
      "select-pane -L }",
      "bind -N 'Select the pane to the right of the active pane' -r Right { "
      "select-pane -R }",
      "bind -N 'Set the even-horizontal layout' M-1 { select-layout "
      "even-horizontal }",
      "bind -N 'Set the even-vertical layout' M-2 { select-layout "
      "even-vertical }",
      "bind -N 'Set the main-horizontal layout' M-3 { select-layout "
      "main-horizontal }",
      "bind -N 'Set the main-vertical layout' M-4 { select-layout "
      "main-vertical }",
      "bind -N 'Select the tiled layout' M-5 { select-layout tiled }",
      "bind -N 'Set the main-horizontal-mirrored layout' M-6 { select-layout "
      "main-horizontal-mirrored }",
      "bind -N 'Set the main-vertical-mirrored layout' M-7 { select-layout "
      ...
      "bind -N 'Select the previous window with an alert' M-p { "
      "previous-window -a }",
      "bind -N 'Move the visible part of the window up' -r S-Up { "
      "refresh-client -U 10 }",
      "bind -N 'Move the visible part of the window down' -r S-Down { "
      "refresh-client -D 10 }",
      "bind -N 'Move the visible part of the window left' -r S-Left { "
      "refresh-client -L 10 }",
      "bind -N 'Move the visible part of the window right' -r S-Right { "
      "refresh-client -R 10 }",
      "bind -N 'Reset so the visible part of the window follows the cursor' -r "
      "DC { refresh-client -c }",
      "bind -N 'Resize the pane up by 5' -r M-Up if -F "
      "'#{?floating_pane_flag}' { resizep -D-5 } { resize-pane -U 5 }",
      "bind -N 'Resize the pane down by 5' -r M-Down { resize-pane -D 5 }",
      "bind -N 'Resize the pane left by 5' -r M-Left if -F "
      "'#{?floating_pane_flag}' { resizep -R-5 } { resize-pane -L 5 }",
      "bind -N 'Resize the pane right by 5' -r M-Right resize-pane -R 5",
      "bind -N 'Resize the pane up' -r C-Up if -F '#{?floating_pane_flag}' { "
      "resizep -D-1 } { resize-pane -U }",
      "bind -N 'Resize the pane down' -r C-Down { resize-pane -D }",
      "bind -N 'Resize the pane left' -r C-Left if -F '#{?floating_pane_flag}' "
      "{ resizep -R-1 } { resize-pane -L }",
      "bind -N 'Resize the pane right' -r C-Right { resize-pane -R }",

      /* Menu keys */
      "bind -N 'Display window menu' < { display-menu -xW -yW -T "
      "'#[align=centre]#{window_index}:#{window_name}' " DEFAULT_WINDOW_MENU
      " }",
      "bind -N 'Display pane menu' > { display-menu -xP -yP -T "
      "'#[align=centre]#{pane_index} (#{pane_id})' " DEFAULT_PANE_MENU " }",
      ...
      "bind -n MouseDrag1Pane { if -F "
      "'#{||:#{pane_in_mode},#{mouse_any_flag}}' { send -M } { copy-mode -M } "
      "}",
      ...
      "bind -n MouseDown2Pane { select-pane -t=; if -F "
      "'#{||:#{pane_in_mode},#{mouse_any_flag}}' { send -M } { paste -p } }",

      /* Mouse button 1 double click on pane. */
      "bind -n DoubleClick1Pane { select-pane -t=; if -F "
      "'#{||:#{pane_in_mode},#{mouse_any_flag}}' { send -M } { copy-mode -H; "
      "send -X select-word; run -d0.3; send -X copy-pipe-and-cancel } }",

      /* Mouse button 1 triple click on pane. */
      "bind -n TripleClick1Pane { select-pane -t=; if -F "
      "'#{||:#{pane_in_mode},#{mouse_any_flag}}' { send -M } { copy-mode -H; "
      "send -X select-line; run -d0.3; send -X copy-pipe-and-cancel } }",
      ...
      "bind -n MouseDown1Control9 { display-menu -t= -xM -yM -O -T 'Kill pane "
      "#{pane_index}?' 'Yes' 'y' { kill-pane -t= } 'No' 'n' {}}",
      ...
      "bind -n MouseDown3StatusLeft { run -C \"display-menu -t= -xM -yW -T "
      "'#[align=centre]#{session_name}' " DEFAULT_SESSION_MENU "\" }",
      "bind -n M-MouseDown3StatusLeft { run -C \"display-menu -t= -xM -yW -T "
      "'#[align=centre]#{session_name}' " DEFAULT_SESSION_MENU "\" }",

      /* Mouse button 3 down on status line. */
      "bind -n MouseDown3Status { display-menu -t= -xW -yW -T "
      "'#[align=centre]#{window_index}:#{window_name}' " DEFAULT_WINDOW_MENU
      "}",
      "bind -n M-MouseDown3Status { display-menu -t= -xW -yW -T "
      "'#[align=centre]#{window_index}:#{window_name}' " DEFAULT_WINDOW_MENU
      "}",

      /* Mouse button 3 down on pane. */
      "bind -n MouseDown3Pane { if -Ft= "
      "'#{||:#{mouse_any_flag},#{&&:#{pane_in_mode},#{?#{m/"
      "r:(copy|view)-mode,#{pane_mode}},0,1}}}' { select-pane -t=; send -M } { "
      "display-menu -t= -xM -yM -T '#[align=centre]#{pane_index} "
      "(#{pane_id})' " DEFAULT_PANE_MENU " } }",
      "bind -n M-MouseDown3Pane { display-menu -t= -xM -yM -T "
      "'#[align=centre]#{pane_index} (#{pane_id})' " DEFAULT_PANE_MENU " }",

      /* Mouse on scrollbar. */
      "bind -n MouseDown1ScrollbarUp { if -Ft= '#{pane_in_mode}' { send -X "
      "page-up } {copy-mode -u } }",
      "bind -n MouseDown1ScrollbarDown { if -Ft= '#{pane_in_mode}' { send -X "
      "page-down } {copy-mode -d } }",
      "bind -n MouseDrag1ScrollbarSlider { if -Ft= '#{pane_in_mode}' { send -X "
      "scroll-to-mouse } { copy-mode -S } }",
      ...
      "bind -Tcopy-mode C-r { command-prompt -P -T search -ip'(search up)' "
      "-I'#{pane_search_string}' { send -X search-backward-incremental -- '%%' "
      "} }",
      "bind -Tcopy-mode C-s { command-prompt -P -T search -ip'(search down)' "
      ...
      "bind -Tcopy-mode F { command-prompt -P -1p'(jump backward)' { send -X "
      "jump-backward -- '%%' } }",
      "bind -Tcopy-mode N { send -X search-reverse }",
      "bind -Tcopy-mode P { send -X toggle-position }",
      "bind -Tcopy-mode R { send -X rectangle-toggle }",
      "bind -Tcopy-mode T { command-prompt -P -1p'(jump to backward)' { send "
      "-X jump-to-backward -- '%%' } }",
      "bind -Tcopy-mode X { send -X set-mark }",
      "bind -Tcopy-mode f { command-prompt -P -1p'(jump forward)' { send -X "
      "jump-forward -- '%%' } }",
      "bind -Tcopy-mode g { command-prompt -P -p'(goto line)' { send -X "
      "goto-line -- '%%' } }",
      "bind -Tcopy-mode n { send -X search-again }",
      "bind -Tcopy-mode q { send -X cancel }",
#ifdef ENABLE_CAPTURE_MODE
      "bind -Tcopy-mode Y { send -X capture }",
#endif
      "bind -Tcopy-mode r { send -X refresh-toggle }",
      "bind -Tcopy-mode t { command-prompt -P -1p'(jump to forward)' { send -X "
      "jump-to-forward -- '%%' } }",
      ...
      "run -d0.3; send -X copy-pipe-and-cancel }",
      ...
      /* Copy mode (vi) keys. */
      "bind -Tcopy-mode-vi '#' { send -FX search-backward -- "
      "'#{copy_cursor_word}' }",
      "bind -Tcopy-mode-vi * { send -FX search-forward -- "
      "'#{copy_cursor_word}' }",
      ...
      "bind -Tcopy-mode-vi / { command-prompt -P -T search -p'(search down)' { "
      ...
      "'%%' } }",
      "bind -Tcopy-mode-vi : { command-prompt -P -p'(goto line)' { send -X "
      "goto-line -- '%%' } }",
      "bind -Tcopy-mode-vi \\; { send -X jump-again }",
      "bind -Tcopy-mode-vi ? { command-prompt -P -T search -p'(search up)' { "
      ...
      "bind -Tcopy-mode-vi F { command-prompt -P -1p'(jump backward)' { send "
      "-X jump-backward -- '%%' } }",
      ...
      "bind -Tcopy-mode-vi f { command-prompt -P -1p'(jump forward)' { send -X "
      "jump-forward -- '%%' } }",
...
#ifdef ENABLE_CAPTURE_MODE
      "bind -Tcopy-mode-vi Y { send -X capture }",
#endif
      "bind -Tcopy-mode-vi r { send -X refresh-toggle }",
      "bind -Tcopy-mode-vi t { command-prompt -P -1p'(jump to forward)' { send "
      "-X jump-to-forward -- '%%' } }",
      ...
      "bind -Tcopy-mode-vi MouseDrag1Pane { select-pane; send -X "
      "begin-selection }",
      "bind -Tcopy-mode-vi MouseDragEnd1Pane { send -X copy-pipe-and-cancel }",
      "bind -Tcopy-mode-vi WheelUpPane { select-pane; send -N5 -X scroll-up }",
      "bind -Tcopy-mode-vi WheelDownPane { select-pane; send -N5 -X "
      "scroll-down }",
      "bind -Tcopy-mode-vi DoubleClick1Pane { select-pane; send -X "
      "select-word; run -d0.3; send -X copy-pipe-and-cancel }",
      "bind -Tcopy-mode-vi TripleClick1Pane { select-pane; send -X "
      "select-line; run -d0.3; send -X copy-pipe-and-cancel }",
...
static enum cmd_retval key_bindings_read_only(struct cmdq_item *item,
                                              __unused void *data) {
  cmdq_error(item, "client is read-only");
  return (CMD_RETURN_ERROR);
}

struct cmdq_item *key_bindings_dispatch(struct key_binding *bd,
...
int key_bindings_has_repeat(struct key_binding **l, u_int n) {
  u_int i;
