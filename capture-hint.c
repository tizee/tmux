 ...
 */
static void capture_hint_decode(char *buf, unsigned long index, int len,
 ...
 */
struct capture_hint *capture_hint_generate(int num_hints, const char *keys,
                                           int *count) {
  ...
  short_len = len - 1;
  n_short =
      (int)((cap - (unsigned long)num_hints) / (unsigned long)(nkeys - 1));
  ...
  while (j < num_hints) {
    ...
    for (idx = 0; idx < (unsigned long)nkeys && j < num_hints; idx++) {
...
void capture_hint_free(struct capture_hint *hints, int count) {
  int i;
